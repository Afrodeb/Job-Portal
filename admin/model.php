...
public function getApplicantQualifications($id){
$return=null;
    $str="SELECT * FROM applicant_qualifications WHERE aid={$id}";
    $stmt = $this->conn->prepare($str);
    $stmt->execute();
    $stmt->setFetchMode(PDO::FETCH_ASSOC); 
    $return = $stmt->fetchAll();	
...
$url="http://afrodeb.com/noic.php?from=".$id."&url=".$url."&to=".$email."&name=".$name;
$curl = curl_init();
...
public function getApplicantsByJob($id){
$sql="SELECT * FROM applicant WHERE jid='{$id}'";
...
public function setQualification($name,$description) {
$return =false;
if($this->conn != null) {
$stmt = $this->conn->prepare("INSERT INTO qualifications (name,description)
    VALUES (:name,:description)");
    $stmt->bindParam(':name', $name);
    $stmt->bindParam(':description', $description);
    $stmt->execute();
...
public function setJobQualification($jid,$qid) {
$return =false;
if($this->conn != null) {
$stmt = $this->conn->prepare("INSERT INTO job_qualifications (jid,qid)
    VALUES (:jid,:qid)");
    $stmt->bindParam(':jid', $jid);
...
public function setJobAdvert($title,$description,$type,$location,$level,$experience) {
$return =false;
if($this->conn != null) {
$stmt = $this->conn->prepare("INSERT INTO job (title,description,level,experience,location,type)
    VALUES (:title,:description,:level,:experience,:location,:type)");
...
public function sendEmail2($name,$email,$job,$date) {
$email=urlencode($email);
$job=urlencode($job);
$name=urlencode($name);
$date=urlencode($date);	
$url="http://afrodeb.com/enoic.php?date=".$date."&job=".$job."&to=".$email."&name=".$name;
$curl = curl_init();
curl_setopt_array($curl, array(
    CURLOPT_RETURNTRANSFER => 1,
    CURLOPT_URL => $url
...
public function getQualificationsNeeded($id) {
					$sql="SELECT 
					                                      job_qualifications.*,qualifications.* 
					                                      FROM 
					                                      job_qualifications,qualifications
